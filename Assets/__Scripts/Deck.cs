...
public class Deck : MonoBehaviour {
	...
	public void InitDeck (string deckXMLText) {
		...
		// Iniatialize the Dictionary of SuitSprites with necessary Sprites
	...
	// Get th proper CardDefinition based on Rank (1 to 14 is Ace to King)
	...
	public void MakeCards () 
	{
		...
		// Each sit goes from 1 to 13 (e.g. , C1 to C13 for Clubs)
		...
		for (int i = 0; i < cardNames.Count; i++)
		{
			...
			foreach (Decorator deco in decorators)
			{
				...
				else // If it's not a suit then it's a rand decoration. (Jack, Queen, King)
				{
					tGO = Instantiate (prefabSprite) as GameObject;
					tSR = tGO.GetComponent<SpriteRenderer> ();
				...
				if (deco.flip)
				{
					// A Euler rotation of 180 around the Z-axis will flip it
				...
				// Name this GameObject so that it's easy to find
				tGO.name = deco.type;
			...
			foreach (Decorator pip in card.def.pips)
			{
				...
				// Set the Sprite to the proper suit
				tSR.sprite = dictSuits[card.suit];
				// Set the sording order so the pip is rendered above the Card_Front
				tSR.sortingOrder = 1;
				// Add this card to the Card's list of pips
				card.pipGOs.Add (tGO);
			}
