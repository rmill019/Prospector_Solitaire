...
public class FloatingScore : MonoBehaviour {
	...
	{
		// When this callback is called by SendMessage add the score from the FloatingScore
		score += fs.score;
	}
	...
	void Update () {
		...
		{
			if (u >= 1)		// If u >= 1 then we are done moving
			{
				uC = 1;		// Set uC = 1 so that we don't overshoot our target
				state = FSState.post;
			...
			// UseBezier curve to move this to the right point
